...
# ---------- Global cache for pincode coordinates ----------
pincode_cache = {}
...
# Function to get coordinates from location name (state level)
def get_coordinates_from_location(location):
    try:
        geolocator = Nominatim(user_agent="CropMainDashboard/1.0 (contact@example.com)")
...
# Function to get coordinates from pincode with retry mechanism + caching
def get_coordinates_from_pincode(pincode):
    ...
    try:
        ...
        headers = {"User-Agent": "CropMainDashboard/1.0 (contact@example.com)"}
        ...
        if response.status_code == 200:
            ...
            if data:
                ...
                pincode_cache[pincode] = coords  # save in cache
                return coords
            # ❌ removed st.warning here (we handle later)
    except Exception as e:
        st.warning(f"Error occurred while getting coordinates: {e}")
    return None
...
def main():
    ...
    if visualize_button:
        coordinates = None
        missing_pincodes = []  # ✅ collect missing pincodes here
        ...
        if coordinates:
            ...
            # Setup marker cluster once (not inside loop)
            marker_cluster = MarkerCluster().add_to(m) if use_marker_cluster else None
            ...
            if is_pincode_selected:
                ...
                if bc_coordinates and not pincode_data.empty:
                    ...
                    missing_pincodes.append(pincode_input)
            ...
            # ✅ Show one consolidated warning for missing pincodes
            if missing_pincodes:
                st.warning(
                    f"No coordinates found for {len(missing_pincodes)} pincodes. "
                    f"Examples: {', '.join(map(str, missing_pincodes[:5]))}..."
                )
